...
const WindowClone = new Lang.Class({
    ...
    _init : function(realWindow) {
        ...
        this._positionChangedId = this.realWindow.connect('position-changed', Lang.bind(this, this._onPositionChanged));
        this._realWindowDestroyedId = this.realWindow.connect('destroy', Lang.bind(this, this._disconnectRealWindowSignals));
...
const WorkspaceThumbnail = new Lang.Class({
    ...
    _init : function(mediator, metaWorkspace) {
        ...
        for (let i = 0; i < windows.length; i++) {
            let minimizedChangedId = windows[i].meta_window.connect('notify::minimized', Lang.bind(this, this._updateMinimized));
    ...
    _doAddWindow : function(metaWin) {
        ...
        if (this._allWindows.indexOf(metaWin) == -1) {
            let minimizedChangedId = metaWin.connect('notify::minimized', Lang.bind(this, this._updateMinimized));
    ...
    _isMyWindow : function (actor) {
        let win = actor.meta_window;
        return win.located_on_workspace(this.metaWorkspace) && (win.get_monitor() == this.monitorIndex);
    },

    // Tests if @win should be shown in the Overview
    _isOverviewWindow : function (win) {
        let tracker = Shell.WindowTracker.get_default();
        return tracker.is_window_interesting(win.get_meta_window()) && win.get_meta_window().showing_on_its_workspace();
    },
...
const GnoMenuThumbnailsBox = new Lang.Class({
    ...
    acceptDrop: function(source, actor, x, y, time) {
        if (this._dropWorkspace != -1) {
            ...
            windows.forEach(function(win) {
                win.meta_window.change_workspace_by_index(win.get_workspace() + 1, true, time);
            });

            if (isWindow) {
                // ... and bam, a workspace, good as new.
                source.metaWindow.change_workspace_by_index(newWorkspaceIndex, true, time);
            } else if (source.shellWorkspaceLaunch) {
                ...
                Main.wm.keepWorkspaceAlive(global.screen.get_workspace_by_index(newWorkspaceIndex), WORKSPACE_KEEP_ALIVE_TIME);
            }
    ...
    createThumbnails: function() {
        ...
        this._syncStackingId = Main.overview.connect('windows-restacked', Lang.bind(this, this._syncStacking));
    ...
    _workspacesChanged: function() {
        ...
        if (newNumWorkspaces > oldNumWorkspaces) {
            this._addThumbnails(oldNumWorkspaces, newNumWorkspaces - oldNumWorkspaces);
        } else {
            let removedIndex;
    ...
    _addThumbnails: function(start, count) {
        for (let k = start; k < start + count; k++) {
            ...
            let thumbnail = new WorkspaceThumbnail(this._mediator, metaWorkspace);
    ...
    _queueUpdateStates: function() {
        ...
        Meta.later_add(Meta.LaterType.BEFORE_REDRAW, Lang.bind(this, this._updateStates));
